...
test('Landing page shows Tavli - Backgammon', async ({ page }, testInfo) => {
  ...
  await tester.step('landing-page-loaded', {
    ...
    verifications: [
      { spec: 'Title is visible', check: async () => await expect(page.locator('h1')).toHaveText('Tavli - Backgammon') }
    ]
  });
